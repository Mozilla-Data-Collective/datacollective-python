...
def test_strip_archive_suffix_removes_known_extensions(tmp_path):
...
def test_resolve_download_dir_prefers_argument(tmp_path, monkeypatch):
...
def test_resolve_download_dir_uses_env_default(tmp_path, monkeypatch):
